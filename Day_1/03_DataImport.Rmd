...
1 - Go to the `File` menu at the top of R Studio & select `New Project...`.
...
Now click the `More` drop-box next to the cog in the **Files** pane, then choose `Move...`, and move this file into your R Project subdirectory data (i.e. in `~/Day_1/data`).
Call a tutor over if you're not sure.

Once we've got that all setup the same for everyone, we'll be off & running!

Just to check you have everything right, please copy & paste the following line of code into your R Console.
...
All of the manipulation we do will instead be written as code & performed on the R object we create after importing the data.
...
Click the `Browse` button and navigate to the file `data/DGE_Results.tsv`.
...
In short, a) doubles are numbers with something after the decimal point, and b) integers are whole numbers.
...
Once you've done that, click the Import button and see what happens.
...
As you may be able to tell, this is not particularly reproducible and we have just clicked a few menu items & buttons.
...
1. Save this as `DGE_Analysis.Rmd` in your `Day_1` main project folder (i.e. NOT in the data folder).
...
We don't really need that final preview line, so close the data preview and remove the third line.

Now, to really have some fun, in the **Environment** tab, click the broom (![](images/broom.png)) icon.
(Say yes when you get the next dialog)
...
This is why we kept our code, and as you can imagine this step is now easily reproducible.
...
Each R function comes with a defined set of arguments.
...
- `na`: tells R how any missing value are encoded in the file. We don't have any of these today, but sometimes we have data from collaborators with values like `-`, or `#N/A` or `9999` to indicate missing measurements, so this can be very useful.
- `comment`: can be used to let R know what any comment lines begin with. Sometimes we have to load files where meaningful information is included at the start prefaced by a comment symbol such as `#`
- `skip`: If we wish to skip an initial number of lines we can tell R to ignore them here.

Also note that there is a function here called `read_tsv()`.
...
This will be identical to what we originally had, but may be even easier to read back in the future when you're re-reading your code.
