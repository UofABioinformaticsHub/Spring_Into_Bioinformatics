...
Where you've written the number `r nrow(DGE_Results)`, replace this with `` `​r nrow(DGE_Results)` `` (you will need to type this ... for reasons), and recompile your document.
<!-- There is a zero-width space in the in-line code to prevent RStudio from evaluating the function. Please leave it there. -->
...
Now where you have your manually typed column names in your paragraph, add the in-line code `` `​r pander(colnames(DGE_Results))` ``.
...
To make a nicer version of our MD plot, we'd put `logCPM` on the x-axis and `logFC` on the y-axis.
