...
Again, it's very similar to a `data.frame` just with slightly different wrapping paper on it than a `tibble`.
However, it doesn't play nicely with the `dplyr` set of functions, so every time we do come across one that we wish to use `dplyr` functions on, we need to convert to a generic `data.frame` using the function `as.data.frame()`.
This is one of the perils of a large and fragmented open-source community with particular fragments having diverse requirements, for example, those using genomic information have completely different requirements to those analysing social media.
