...
The fundamentl data structure we need to perform Differential Gene Expression analysis, needs gene identifiers as rownames, meaning a `tibble ` is not really viable for this step.
...
This is what we've done when we dropped the `.cbc` and `.skm` extensions from our smaple names.
We matched "SRR[0-9]+" which will capture the sample IDs, but as soon as it hits the period (.) before the cbc/skm text fragment, the match would stop, and we have used `str_extract()` to return only the tet fragment that matches.
