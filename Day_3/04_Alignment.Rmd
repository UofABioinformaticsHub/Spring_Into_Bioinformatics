...
There is another type of file called a SAM (Sequence Alignment/Map) file which is in plain text, but SAM files can become very large and waste our precious storage resources.
...
This tool has a series of commands which we can see if we just type `samtools` into our terminal.
...
These are the aligned positions.
CIGAR strings from RNA-seq read alignments contain large numbers followed by `N`.
This represents a spliced alignment, and in this context is very likely to be where part of a read aligns to one exon, whilst the other part of the read aligns to another exon, making this commonly indicative of a spliced alignment.
...
So far, we have only aligned a single read file. We will now write a script for aligning ***ALL*** of our read files.
